  ...
  return (
    <div className='container mx-auto px-2 md:px-0 '>
        <div className='flex items-center justify-between mb-5 mt-2  '>
            ...
            <button className='flex  items-center gap-1.5 text-xl   p-2 rounded-lg bg-cyan-300 hover:bg-cyan-400 cursor-pointer ' onClick={() => handleThemeSelector() }>
            ...
            <div className=' col-span-12 md:col-span-5 bg-white '>
                <div className='grid grid-cols-2 gap-5 max-h-[80vh] overflow-scroll custom-scrollbar md:pr-5  '>
                        {tabValue === "Templates" && resumeTemplates.map((template, index)=> (
                            <TemplateCard 
                                key = {`template_${index}`}
                                thumnailImg = {template.thumbnailImg}
    ...
    return (
        <div className={` h-28 bg-purple-50 flex rounded-lg overflow-hidden  border-2 ${ isSelected ? " border-purple-500" : "border-none"} `}  >
            { colors.map((color, index) => (
                <div key={`color_${index}`} className=' flex-1 ' style={{ backgroundColor : colors[index]}} onClick={onSelect}  >
